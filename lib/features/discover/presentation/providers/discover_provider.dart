...
import 'package:fantastic_app_riverpod/features/challenges/data/services/challenges_service.dart';
import 'package:fantastic_app_riverpod/features/coaching/data/services/coaching_service.dart';
import 'package:fantastic_app_riverpod/features/guidedactivities/data/services/guided_activities.dart';
...
class JourneysNotifier extends StateNotifier<JourneysState> {
  ...
  Future<void> fetchJourneys() async {
    try {
      AppLogger.i('JourneysNotifier: Starting to fetch journeys...');
      state = state.copyWith(isLoading: true);
      final isParentMode = _ref.read(personaProvider).isParentMode;
      AppLogger.i('JourneysNotifier: Parent mode: $isParentMode');

      final journeys =
          await _journeyService.fetchJourneys(isParentMode: isParentMode);

      AppLogger.i('JourneysNotifier: Fetched ${journeys.length} journeys');

      if (journeys.isNotEmpty) {
        AppLogger.i('JourneysNotifier: Sample journeys:');
        ...
        AppLogger.w('JourneysNotifier: No journeys found!');
      }

      state = state.copyWith(journeys: journeys, isLoading: false);
    } catch (e) {
      AppLogger.e('JourneysNotifier: Error fetching journeys: $e');
      state = state.copyWith(
...
class CoachingNotifier extends StateNotifier<CoachingState> {
  ...
  Future<void> fetchCoaching() async {
    try {
      AppLogger.i('CoachingNotifier: Starting to fetch coaching...');
      state = state.copyWith(isLoading: true);
      final coaching = await _coachingService.getMainCoachings();
      AppLogger.i(
          'CoachingNotifier: Fetched ${coaching.length} coaching items');
      state = state.copyWith(coaching: coaching, isLoading: false);

      // Debug: Print first few items
      if (coaching.isNotEmpty) {
        AppLogger.i('CoachingNotifier: Sample coaching items:');
        ...
        AppLogger.w('CoachingNotifier: No coaching items found!');
      }
    } catch (e) {
      AppLogger.e('CoachingNotifier: Error fetching coaching: $e');
      state = state.copyWith(
...
final currentDataProvider = Provider<List<Map<String, dynamic>>>((ref) {
  ...
  AppLogger.i('currentDataProvider: selectedIndex=$selectedIndex');
